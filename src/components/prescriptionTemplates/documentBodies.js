...
export function renderOrdonnanceBody({
  validMedsToPrint = [],
  freeTextToPrint = "",
  prescriptionsCount,
  ...
  if (!rxHtml.trim()) {
    rxHtml =
      '<div style="font-size: 14px; color: #666; font-style: italic; padding: 20px 0;">Aucun médicament prescrit.</div>';
  }
...
export function renderBilanBody({
  rawList = [],
  documentSubtitle,
  prescriptionsCount,
  lang,
  ...
  if (rawList.length > 0) {
    ...
    listHtml =
      '<div style="font-size: 14px; color: #666; font-style: italic; padding: 20px 0;">Aucun examen sélectionné.</div>';
  }

  const subMsg =
    documentSubtitle ||
    (lang === "ar"
      ? "الرجاء إجراء الفحوصات التالية :"
      : lang === "en"
        ? "Please perform the following tests :"
        : "Faire SVP les bilans suivants :");
...
export function renderCertificatBody({ certificat = {} }) {
  ...
  const content =
    certificat.content ||
    "Je soussigné, Docteur en médecine, certifie avoir examiné ce jour le patient susnommé...";
...
function numberToFrenchWordOnly(n) {
...
export function renderArretTravailBody({ arretTravail = {} }) {
  ...
  const wordOnly = arretTravail.wordOnly || numberToFrenchWordOnly(days);
  const isProlongation = arretTravail.type === 'prolongation' || String(arretTravail.type) === '2';
  const isReprise = arretTravail.type === 'reprise' || String(arretTravail.type) === '3';
    ...
    return `
      <div class="arret-body-container" style="min-height: 200px; padding: 15px 5px;">
        <div style="font-size: 15px; line-height: 1.8; font-family: 'Segoe UI', Arial, sans-serif; color: #000;">
          Je soussigné, Docteur en médecine, certifie que l'état de santé du patient susnommé<br/>
          Lui permet de reprendre son travail à dater<br/>
          du : <strong>${startDate}</strong>
          ${reason ? `<br/><br/><strong>Observation : </strong>${reason}` : ""}
        </div>
      </div>
    `;
  }

  const introText = isProlongation
  ...
  return `
    <div class="arret-body-container" style="min-height: 200px; padding: 15px 5px;">
      <div style="font-size: 15px; line-height: 1.8; font-family: 'Segoe UI', Arial, sans-serif; color: #000;">
        Je soussigné, Docteur en médecine, certifie que l'état de santé du patient susnommé ${introText}<br/><br/>
        <div style="font-size: 16px; font-weight: bold; text-align: center; margin: 15px 0;">
          ${days}${wordOnly ? ` (${wordOnly})` : ''} Jour(s)
        </div>
